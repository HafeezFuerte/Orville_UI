    ...
    this.portfolioService.getMasterByType({
      ...
      next: (res: any) => {
        if (res && res.statusCode === '200' && res.objResult) {
          const detail = res.objResult.table?.[0] || res.objResult.inventory_item?.[0] || {};
          
          this.itemName = detail.item_name || '';
          this.partNumber = detail.part_number || '';
          this.selectedCategory = detail.category || null;
          this.selectedSubcategory = detail.subcategory || null;
          this.detail = detail.notes || detail.detail || '';
          this.placedDate = this.formatDateForInput(detail.placed_date);
          this.expirationDate = this.formatDateForInput(detail.expiry_date);
          this.selectedVendor = detail.vendor_code || null;
          ...
          if (res.objResult.table1 || res.objResult.lines) {
            ...
            this.lines = rawLines.map((l: any) => ({
              ...
              availableQty: String(l.qty || ''),
              minimumQty: String(l.minimum_qty || '')
            }));
    ...
    this.portfolioService.saveInventoryItem(formData).subscribe({
      next: (res) => {
        if (res && (res.statusCode === 200 || res.statusCode === '200' || res.isSuccess)) {
          this.toastr.success(res.message || 'Inventory Item saved successfully');
          this.goBack();
        } else {
          this.toastr.error(res.message || 'Failed to save inventory item');
        }
      },
      error: (err) => {
