...
export class AddTenantComponent implements OnInit {
  ...
  emergencyContacts: Array<{
    ...
    phone: string;
    work_phone: string;
  }> = [{ email: '', name: '', relation: '', phone: '', work_phone: '' }];
  ...
  /** Presentation-only: documents modal (no API). */
  ...
  /** Presentation-only: emergency contacts list (no API). */
  addEmergencyContact() {
    this.emergencyContacts.push({ email: '', name: '', relation: '', phone: '', work_phone: '' });
  }

  removeEmergencyContact(index: number) {
    if (this.emergencyContacts.length <= 1) return;
