...
export class AddLandlordComponent implements OnInit {
  ...
  isEditMode = false;
  landlordId: any = null;
  landlordDbId: number = 0; // stores the real numeric DB id for updates
  ...
  // Modal State (presentation-only documents — no API)
  ...
  get selectedCountryObj() {
    return this.countries.find(c => c.id === Number(this.landlordData.country_id)) || null;
  }
      ...
      next: (res: any) => {
        if (res.statusCode == 200 && res.objResult && res.objResult.table) {
          ...
          if (!this.landlordData.country_id) {
            ...
            if (defaultCountry) {
              this.landlordData.country_id = defaultCountry.id;
  ...
  saveLandlord() {
    ...
    const requestJson = {
      ...
      id: this.isEditMode ? this.landlordDbId : 0,
      ...
      country_id: Number(this.landlordData.country_id) || 0,
  ...
  onDocFileSelected(event: Event): void {
    ...
    this.applyDocFile(file);
    input.value = '';
  }

  onDocFileDrop(event: DragEvent): void {
  ...
  clearDocFile(): void {
    ...
    this.docForm.fileSizeLabel = '';
    this.docForm.uploadProgress = 0;
  }

  private applyDocFile(file: File | null): void {
    if (!file) {
      this.clearDocFile();
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      this.toastr.warning('Maximum file size is 10 MB.', 'Document');
      return;
    }
    ...
    this.docForm.file = file;
    this.docForm.fileName = file.name;
    this.docForm.fileExt = ext;
    this.docForm.fileSizeLabel = this.formatFileSize(file.size);
    this.simulateDocUpload();
  }

  private simulateDocUpload(): void {
    this.clearDocUploadTimer();
    this.docForm.uploadProgress = 0;
    this.docUploadTimer = setInterval(() => {
      this.docForm.uploadProgress = Math.min(100, this.docForm.uploadProgress + 18);
      if (this.docForm.uploadProgress >= 100) {
        this.clearDocUploadTimer();
      }
    }, 120);
  }

  private clearDocUploadTimer(): void {
    if (this.docUploadTimer) {
      clearInterval(this.docUploadTimer);
      this.docUploadTimer = null;
    }
  }
  ...
  private docIconClass(fileName: string): string {
    const lower = fileName.toLowerCase();
    if (lower.endsWith('.pdf')) return 'ri-file-pdf-line';
    if (/\.(png|jpe?g|gif|webp|bmp)$/i.test(lower)) return 'ri-image-line';
    return 'ri-file-text-line';
  }

  private docBadgeFromShares(): { badge: string; badgeClass: string } {
    if (this.docForm.shareWithTenant) {
      return { badge: 'Tenant', badgeClass: 'bg-warning/10 text-warning border-warning/20' };
    }
    if (this.docForm.shareWithLandlord) {
      return { badge: 'Landlord', badgeClass: 'bg-warning/10 text-warning border-warning/20' };
    }
    return { badge: 'Verified', badgeClass: 'bg-success/10 text-success border-success/20' };
  }

  saveDocument(): void {
    if (!this.docForm.type) {
      this.toastr.warning('Please select a document type.', 'Document');
      return;
    }
    if (!this.docForm.file) {
      this.toastr.warning('Please attach a document file.', 'Document');
      return;
    }

    const file = this.docForm.file;
    const { badge, badgeClass } = this.docBadgeFromShares();
    ...
    this.uploadedDocuments = [
      ...this.uploadedDocuments,
      {
        ...
        badge,
        badgeClass,
        iconClass: this.docIconClass(file.name),
        previewUrl: URL.createObjectURL(file)
      }
    ];

    this.toggleDocModal(false);
  }

  viewDocument(doc: { previewUrl?: string; name: string }): void {
    if (doc.previewUrl) {
