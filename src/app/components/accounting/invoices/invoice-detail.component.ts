...
export class InvoiceDetailComponent implements OnInit {
  ...
  printInvoice(): void {
    if (!this.invoice || (!this.invoice.code && !this.invoice.invoice_no)) {
      this.toastr.error('Invoice details not loaded yet');
      return;
    }
  ...
  triggerPrint(): void {
    const printElement = document.querySelector('.printable-invoice-receipt-container');
    if (!printElement) {
      window.print();
      return;
    }
    ...
    if (doc) {
      ...
      doc.write(`
        ...
        <html>
          <head>
            <base href="${origin}/">
            <title>Invoice Receipt - ${this.invoice?.invoice_no || this.invoice?.code}</title>
            <style>
              @page { size: portrait; margin: 10mm; }
              body { font-family: Arial, Helvetica, sans-serif; color: #111; background: #fff; margin: 0; padding: 10px; }
              .printable-invoice-receipt-container { display: block; width: 100%; box-sizing: border-box; }
