...
export class CreateVendorContractComponent implements OnInit {
  ...
  get selectedVendorName(): string {
    ...
    const match = this.vendors.find(v => v.code === this.form.vendor);
    return match ? match.name : this.form.vendor;
  }
  ...
  save(): void {
    ...
    if (!this.form.name && this.form.vendor) {
      this.form.name = `${this.selectedVendorName} - Contract`;
    }
    ...
    const request = {
      ...
      is_per_service_charge: this.parseFeeTypeId(this.form.feeType) === 3,
    ...
    console.log('Sending saveVendorContract payload:', request);
